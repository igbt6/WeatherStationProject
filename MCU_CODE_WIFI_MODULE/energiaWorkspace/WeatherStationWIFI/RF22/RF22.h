...
///
/// \endcode
/// For Stellaris Launchpad EK-LM4F120XL. SPI USED BY DEFAULT: MODULE 3
/// \code
///                 Launchpad    RFM-22B
///                 GND----------GND-\ (ground in)
///                              SDN-/ (shutdown in)
///                 3V3----------VCC   (3.3V in)
///  INTERRUPT pin PE_3----------NIRQ  (interrupt request out)
///         SS pin PE_1----------NSEL  (chip select in)
///        SCK pin PD_0----------SCK   (SPI clock in)
///       MOSI pin PD_3----------SDI   (SPI Data in)
///       MISO pin PD_2----------SDO   (SPI data out)
///   SHUTDOWN pin PE_2----------SDN
...
public:
    ...
    /// \param[in] shutdown Pin for radio , should be hold on 0 (GND) , 1 (VCC) - shutdown mode
    /// \param[in] interrupt The interrupt number to use. 0 - 2. Default is interrupt 0 (Arduino input pin 2)
    /// \param[in] spi Pointer to the SPI interface object to use. 
    ///                Defaults to the standard Arduino hardware SPI interface
    RF22(uint8_t slaveSelectPin, uint8_t shutdownPin, uint8_t interruptPin = 0, GenericSPIClass *spi = &HardwareSpiRFM23);
...
protected:
    ...
    uint8_t 			_shutdownPin; // SDN should be = 0 in all modes except Shutdown mode. When SDN =1 the chip will be completely shutdown and the contents of the registers will be lost
    uint8_t             _interrupt;
    uint8_t             _deviceType;
